...
# 创建 Blueprint 实例
det_coco_bp = Blueprint('cocoDetect', __name__)


@det_coco_bp.route('/', methods=['POST'])
def get_coco_object():
    ...
    service_result = {
        ...
        'conf': [],  # 置信度：float[1]
        'cls': [],  # 类别：int[1]
        'cost_time': {'total': 0.0, 'image_read': 0.0, 'coco_det': 0.0},
        'info': '',  # 描述信息：str
    ...
    try:
        ...
        service_result['cost_time']['coco_det'] = result['cost_time']
