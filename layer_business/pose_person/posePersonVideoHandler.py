...
# 路由：初始化并获取视频流地址
@pose_person_bp.route('/v_init', methods=['POST'])
def init_detection():
...
# 路由：开始检测
@pose_person_bp.route('/v_start', methods=['POST'])
def start_detection():
    global processing
    if video_url:
        with condition:
...
# 路由：停止检测
@pose_person_bp.route('/v_stop', methods=['POST'])
def stop_detection():
    global processing
    processing = False
...
# 路由：结束视频流读取
@pose_person_bp.route('/v_end', methods=['POST'])
def end_stream():
...
# 路由：获取检测结果
@pose_person_bp.route('/v_results', methods=['GET'])
def get_results():
    def generate():
