...
from layer_infer import pose_person
from utils.ops_image import get_image

# 创建 Blueprint 实例
pose_person_bp = Blueprint('posePerson', __name__)


@pose_person_bp.route('/', methods=['POST'])
def get_coco_object():
    ...
    service_result = {
        ...
        'bbox': [],  # 检测框：xyxy int[4]
        'conf': [],  # 置信度：float[1]
        'keypoint': [],  # 关键点：float[51]
        'cost_time': {'total': 0.0, 'image_read': 0.0, 'person_pose': 0.0},
        'info': '',  # 描述信息：str
    ...
    try:
        ...
        service_result['cost_time']['person_pose'] = result['cost_time']
