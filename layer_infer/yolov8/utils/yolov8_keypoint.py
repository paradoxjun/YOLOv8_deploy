...
class YOLOv8KPT(YOLOv8):
    def __init__(self, model_path, kpt_shape=(17, 3), img_size=(640, 640), conf_thres=0.6, iou_thres=0.7):
        ...
        self.kpt = np.zeros(self.kpt_shape)  # 初始化单个人体关键点实例
        # self.kpt_class_name = class_name  # 关键点所属类别名
    ...
    def pipeline(self, image):
        ...
        try:
            self.boxes, self.scores, self.kpt = self.process_output(outputs)
    ...
    def process_output(self, output):
        ...
        # print('debug -> process_output kpts.shape{}  boxes.shape{} kpts{} boxes{}'.format(kpts.shape, boxes.shape,
        # kpts, boxes))
    ...
    def extract_boxes_and_kpts(self, predictions):
        boxes = self.extract_boxes(predictions[:, :4])
        kpts = self.rescale_coords(predictions[:, 5:])

        return boxes, kpts

    def rescale_coords(self, kpts_):
        kpts_[:, ::3] = kpts_[:, ::3] * self.img_width / self.input_width
        kpts_[:, 1::3] = kpts_[:, 1::3] * self.img_height / self.input_height

        return kpts_

        # print(f"keypoints.shape: {kpts_.shape}")
        # kpts_ = np.reshape(kpts_, (-1, *self.kpt_shape))
        # kpts_[:, :, 0] = kpts_[:, :, 0] * self.img_width / self.input_width
        # kpts_[:, :, 1] = kpts_[:, :, 1] * self.img_height / self.input_height
...
if __name__ == '__main__':
    ...
    pose_predictor = YOLOv8KPT(model_path, conf_thres=0.3, iou_thres=0.5)
